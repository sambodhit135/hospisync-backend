...
file_path = 'D:/Porject using antigravity/Hospisync-backend/Hospisync-backend/src/main/resources/static/js/transfer.js'
...
for i, line in enumerate(lines):
    if '// ==========================================' in line and 'SENDER-SIDE TIMER' in lines[i+1]:
        # We found the start of the previous appended code. Stop processing original lines.
        break
        
    if 'if (result && !result.error) {' in line and 'showToast' in lines[max(0, i+1)]:
        # Instead of replacing just one line, we inject the new block and skip until the else block
        new_block = """        if (result && !result.error) {
            ...
            setTimeout(() => {
                ...
                if (typeof initTransferPage === 'function') setTimeout(initTransferPage, 100);
            }, 500);
"""
...
# Append the new user-provided bottom logic
user_logic = """
...
// Call on page load
// Use a generic logic that is safe to add
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initTransferPage);
} else {
    initTransferPage();
}
// Add hash change listener to trigger setup when clicking internal links
...
async function pollTransferStatus(transferId) {
  try {
    ...
    // Update timer panel based on stage
...
function updateTimerPanel(transfer) {
  ...
  if (stage === 'PENDING') {
    ...
    return;
  } else if (stage === 'TIMEOUT_STAGE1' || 
             stage === 'TIMEOUT_STAGE2' ||
             stage === 'REJECTED') {
  ...
  panel.innerHTML = `
    <div style="background:white;
      ...
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <div>
          <span style="background:#EFF6FF; color:#004ac6; padding:4px 12px; border-radius:20px; font-size:12px; font-weight:600;">
            ${stageText}
          </span>
          <h3 style="margin:8px 0 4px; font-size:18px; color:#0F172A;">
            Transfer sent to: ${transfer.toHospitalName || 'Network Facility'}
          </h3>
          <p style="color:#64748B; margin:0; font-size:14px;">
            ${transfer.totalPatients || 0} patients
          </p>
        </div>
        
        <div style="text-align:center;">
          <div style="font-size:52px; font-weight:700; color:${timerColor}; font-family:monospace; ${remainingSecs < 30 ? 'animation:pulse 1s infinite' : ''}">
            ${timeStr}
          </div>
  ...
  panel.innerHTML = `
    <div style="background:#DCFCE7;
      ...
      <p style="color:#166534; margin:0;">
        Hospital: ${transfer.toHospitalName || 'Network Facility'} 
        has accepted the transfer.
        ${transfer.assignedDoctorName ? 'Doctor: ' + transfer.assignedDoctorName : ''}
      </p>
    </div>`;
  ...
  if (!next) {
    panel.innerHTML = `
      <div style="background:#FEF3C7;
                  ...
                  padding:24px;
                  margin-bottom:24px;">
        <h3 style="color:#B45309; margin:0 0 8px;">
          ⚠️ No Response Received
        </h3>
        <p style="color:#92400E; margin:0;">
          ${transfer.toHospitalName || 'Hospital'} did not respond.
          No other hospitals available nearby.
  ...
  panel.innerHTML = `
    <div style="background:white;
      ...
      <div style="margin-bottom:16px;">
        <span style="background:#FEF3C7; color:#B45309; padding:4px 12px; border-radius:20px; font-size:12px; font-weight:600;">
          ⌛ ${transfer.toHospitalName || 'Hospital'} did not respond
        </span>
      </div>
      
      <h3 style="margin:0 0 4px;color:#0F172A;">
        🔄 Next Available Hospital Found:
      </h3>
      ...
      <div style="background:#F8FAFC; border-radius:8px; padding:16px; margin:12px 0;">
        ...
        <div style="color:#64748B; font-size:14px; margin-top:4px;">
          📍 ${next.distanceKm || 0} km away &nbsp;|&nbsp; 🛏️ ${next.availableBeds || 0} beds
          ${next.availableDoctorName ? ' | 👨⚕️ ' + next.availableDoctorName : ''}
        </div>
      </div>
      
      <div style="display:flex; gap:12px;">
        <button 
          onclick="sendToNextHospital(${next.hospitalId}, '${next.hospitalName || 'Next Hospital'}', ${transfer.totalPatients || 1})"
          ...
          Send Request to ${next.hospitalName}
        </button>
        <button 
...
new_lines.append(user_logic)

with codecs.open(file_path, 'w', encoding='utf-8') as f:
    f.writelines(new_lines)

print("transfer.js updated successfully")
